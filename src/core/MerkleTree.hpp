...
static inline uint32_t convert_endianness(uint32_t n) {
    ...
    // Just as fast.
    uint32_t r = 0;
    for (size_t i = 0; i < sz; i++) r |= ((n >> (8 * ((sz - 1) - i))) & 0xFF) << (8 * i);
    return *reinterpret_cast<uint32_t*>(&r);
#else
    // A little slower, but works for both endiannesses.
    uint8_t r[8];
    for (size_t i = 0; i < sz; i++) r[i] = (n >> (8 * ((sz - 1) - i))) & 0xFF;
    return *reinterpret_cast<uint32_t*>(&r);
#endif
}
...
static inline void serialise_uint64_t(uint64_t n, std::vector<uint8_t>& bytes) {
    ...
    for (uint64_t i = 0; i < sz; i++) bytes.push_back((n >> (8 * (sz - i - 1))) & 0xFF);
}

static inline uint64_t deserialise_uint64_t(const std::vector<uint8_t>& bytes, size_t& index) {
    uint64_t r = 0;
    uint64_t sz = sizeof(uint64_t);
    for (uint64_t i = 0; i < sz; i++) r |= static_cast<uint64_t>(bytes[index++]) << (8 * (sz - i - 1));
    return r;
}
...
struct HashT {
    ...
    HashT<SIZE>(const uint8_t* bytes) { std::copy(bytes, bytes + SIZE, this->bytes); }

    /// @brief Constructs a Hash from a string
    /// @param s String to read the hash value from
    HashT<SIZE>(const std::string& s) {
        if (s.length() != 2 * SIZE) throw std::runtime_error("invalid hash string");
    ...
    HashT<SIZE>(const std::vector<uint8_t>& bytes) {
        if (bytes.size() < SIZE) throw std::runtime_error("not enough bytes");
        deserialise(bytes);
    }
    ...
    HashT<SIZE>(const std::vector<uint8_t>& bytes, size_t& position) {
        if (bytes.size() - position < SIZE) throw std::runtime_error("not enough bytes");
        deserialise(bytes, position);
    }

    /// @brief Deserialises a Hash from an array of bytes
    /// @param bytes Array to read the hash value from
    HashT<SIZE>(const std::array<uint8_t, SIZE>& bytes) { std::copy(bytes.data(), bytes.data() + SIZE, this->bytes); }
    ...
    std::string to_string(size_t num_bytes = SIZE, bool lower_case = true) const {
        ...
        for (size_t i = 0; i < num_bytes; i++)
            snprintf(const_cast<char*>(r.data() + 2 * i), num_chars + 1 - 2 * i, lower_case ? "%02x" : "%02X",
                     bytes[i]);
        return r;
    }
    ...
    bool operator==(const HashT<SIZE>& other) const { return memcmp(bytes, other.bytes, SIZE) == 0; }
    ...
    void deserialise(const std::vector<uint8_t>& buffer, size_t& position) {
        ...
        if (buffer.size() - position < SIZE) throw std::runtime_error("not enough bytes");
        for (size_t i = 0; i < sizeof(bytes); i++) bytes[i] = buffer[position++];
    }
...
class PathT {
   public:
    ...
    /// @param bytes Vector to deserialise from
    /// @param position Position of the first byte in @p bytes
    PathT(const std::vector<uint8_t>& bytes, size_t& position) { deserialise(bytes, position); }
    ...
    std::shared_ptr<HashT<HASH_SIZE>> root() const {
        std::shared_ptr<HashT<HASH_SIZE>> result = std::make_shared<HashT<HASH_SIZE>>(_leaf);
        MERKLECPP_TRACE(MERKLECPP_TOUT << "> PathT::root " << _leaf.to_string(TRACE_HASH_SIZE) << std::endl);
        for (const Element& e : elements) {
            if (e.direction == PATH_LEFT) {
                MERKLECPP_TRACE(MERKLECPP_TOUT << " - " << e.hash.to_string(TRACE_HASH_SIZE) << " x "
                                               << result->to_string(TRACE_HASH_SIZE) << std::endl);
                HASH_FUNCTION(e.hash, *result, *result);
            } else {
                MERKLECPP_TRACE(MERKLECPP_TOUT << " - " << result->to_string(TRACE_HASH_SIZE) << " x "
                                               << e.hash.to_string(TRACE_HASH_SIZE) << std::endl);
                HASH_FUNCTION(*result, e.hash, *result);
            }
        }
        MERKLECPP_TRACE(MERKLECPP_TOUT << " = " << result->to_string(TRACE_HASH_SIZE) << std::endl);
        return result;
    }
    ...
    /// expected to hash to.
    bool verify(const HashT<HASH_SIZE>& expected_root) const { return *root() == expected_root; }
    ...
    size_t serialised_size() const { return sizeof(_leaf) + elements.size() * sizeof(Element); }
    ...
    const HashT<HASH_SIZE>& operator[](size_t i) const { return std::next(begin(), i)->hash; }
    ...
    std::string to_string(size_t num_bytes = HASH_SIZE, bool lower_case = true) const {
        ...
        for (auto& e : elements)
            stream << " " << e.hash.to_string(num_bytes, lower_case) << (e.direction == PATH_LEFT ? "(L)" : "(R)");
        return stream.str();
    }
    ...
    bool operator==(const PathT<HASH_SIZE, HASH_FUNCTION>& other) const {
        if (_leaf != other._leaf || elements.size() != other.elements.size()) return false;
        auto it = elements.begin();
        auto other_it = other.elements.begin();
        while (it != elements.end() && other_it != other.elements.end()) {
            if (it->hash != other_it->hash || it->direction != other_it->direction) return false;
            it++;
            other_it++;
        }
        return true;
    ...
    bool operator!=(const PathT<HASH_SIZE, HASH_FUNCTION>& other) { return !this->operator==(other); }
...
class TreeT {
   protected:
    ...
    struct Node {
        ...
        static Node* copy_node(const Node* from, std::vector<Node*>* leaf_nodes = nullptr,
                               size_t* num_flushed = nullptr, size_t min_index = 0, size_t max_index = SIZE_MAX,
                               size_t indent = 0) {
            ...
            r->dirty = from->dirty;
            r->left = copy_node(from->left, leaf_nodes, num_flushed, min_index, max_index, indent + 1);
            r->right = copy_node(from->right, leaf_nodes, num_flushed, min_index, max_index, indent + 1);
   ...
   public:
    ...
    TreeT(const std::vector<uint8_t>& bytes, size_t& position) { deserialise(bytes, position); }
    ...
    ~TreeT() {
        delete (_root);
        for (auto n : uninserted_leaf_nodes) delete (n);
    }
    ...
    void insert(const Hash& hash) {
        MERKLECPP_TRACE(MERKLECPP_TOUT << "> insert " << hash.to_string(TRACE_HASH_SIZE) << std::endl;);
    ...
    void insert(const std::vector<Hash>& hashes) {
        for (auto hash : hashes) insert(hash);
    }
    ...
    void flush_to(size_t index) {
        ...
        walk_to(index, false, [this](Node*& n, bool go_right) {
            if (go_right && n->left) {
                MERKLECPP_TRACE(MERKLECPP_TOUT << " - conflate " << n->left->hash.to_string(TRACE_HASH_SIZE)
                                               << std::endl;);
                if (n->left && n->left->dirty) hash(n->left);
                delete (n->left->left);
                n->left->left = nullptr;
                delete (n->left->right);
    ...
    void retract_to(size_t index) {
        ...
        if (index < min_index()) throw std::runtime_error("leaf index out of bounds");
        ...
        Node* new_leaf_node = walk_to(index, true, [this](Node*& n, bool go_right) {
            ...
            n->dirty = true;
            if (go_left && n->right) {
                MERKLECPP_TRACE(MERKLECPP_TOUT << " - eliminate " << n->right->hash.to_string(TRACE_HASH_SIZE)
                                               << std::endl;);
                ...
                *n = *old_left;
                ...
                if (n->left && n->right) n->dirty = true;

                if (is_root) {
                    MERKLECPP_TRACE(MERKLECPP_TOUT << " - new root: " << n->hash.to_string(TRACE_HASH_SIZE)
                                                   << std::endl;);
                    assert(_root == n);
                }
    ...
    Tree& operator=(const Tree& other) {
        ...
        _root = Node::copy_node(other._root, &leaf_nodes, &to_skip, other.min_index(), other.max_index());
        for (auto n : other.uninserted_leaf_nodes) uninserted_leaf_nodes.push_back(Node::copy_node(n));
        num_flushed = other.num_flushed;
    ...
    const Hash& root() {
        ...
        MERKLECPP_TRACE(MERKLECPP_TOUT << " - root: " << _root->hash.to_string(TRACE_HASH_SIZE) << std::endl;);
        return _root->hash;
    }
    ...
    std::shared_ptr<Hash> past_root(size_t index) {
        ...
        MERKLECPP_TRACE(MERKLECPP_TOUT << " - " << p->to_string(TRACE_HASH_SIZE) << std::endl;
                        MERKLECPP_TOUT << " - " << result->to_string(TRACE_HASH_SIZE) << std::endl;);

        for (auto e : *p)
            if (e.direction == Path::Direction::PATH_LEFT) HASH_FUNCTION(e.hash, *result, *result);
    ...
    inline Node* walk_to(size_t index, bool update, const std::function<bool(Node*&, bool)>&& f) {
        if (index < min_index() || max_index() < index) throw std::runtime_error("invalid leaf index");
        ...
        Node* cur = _root;
        size_t it = 0;
        if (_root->height > 1) it = index << (sizeof(index) * 8 - _root->height + 1);
        assert(walk_stack.empty());

        for (uint8_t height = _root->height; height > 1;) {
            assert(cur->invariant());
            bool go_right = (it >> (8 * sizeof(it) - 1)) & 0x01;
            if (update) walk_stack.push_back(cur);
            MERKLECPP_TRACE(MERKLECPP_TOUT << " - at " << cur->hash.to_string(TRACE_HASH_SIZE) << " (" << cur->size
                                           << "/" << (unsigned)cur->height << ")"
                                           << " (" << (go_right ? "R" : "L") << ")" << std::endl;);
            if (cur->height == height) {
                if (!f(cur, go_right)) continue;
    ...
    std::shared_ptr<Path> path(size_t index) {
        ...
        return std::make_shared<Path>(leaf_node(index)->hash, index, std::move(elements), max_index());
    }
    ...
    std::shared_ptr<Path> past_path(size_t index, size_t as_of) {
        MERKLECPP_TRACE(MERKLECPP_TOUT << "> past_path from " << index << " as of " << as_of << std::endl;);
        statistics.num_past_paths++;

        if ((index < min_index() || max_index() < index) || (as_of < min_index() || max_index() < as_of) ||
            index > as_of)
        ...
        std::list<typename Path::Element> root_to_fork, fork_to_index, fork_to_as_of;
        ...
        for (uint8_t height = _root->height; height > 1;) {
            ...
            MERKLECPP_TRACE(MERKLECPP_TOUT << " - at " << (unsigned)height << ": "
                                           << cur_i->hash.to_string(TRACE_HASH_SIZE) << " (" << cur_i->size << "/"
                                           << (unsigned)cur_i->height << "/" << (go_right_i ? "R" : "L") << ")"
                                           << " / " << cur_a->hash.to_string(TRACE_HASH_SIZE) << " (" << cur_a->size
                                           << "/" << (unsigned)cur_a->height << "/" << (go_right_a ? "R" : "L") << ")"
                                           << std::endl;);

            if (!fork_node && go_right_i != go_right_a) {
                assert(cur_i == cur_a);
                assert(!go_right_i && go_right_a);
                MERKLECPP_TRACE(MERKLECPP_TOUT << " - split at " << cur_i->hash.to_string(TRACE_HASH_SIZE)
                                               << std::endl;);
                fork_node = cur_i;
            }
        ...
        MERKLECPP_TRACE({
            ...
            for (auto e : root_to_fork)
                MERKLECPP_TOUT << " " << e.hash.to_string(TRACE_HASH_SIZE) << "(" << e.direction << ")";
        ...
        if (!fork_to_index.empty()) fork_to_index.pop_front();
        for (auto it = fork_to_index.rbegin(); it != fork_to_index.rend(); it++) path.push_back(std::move(*it));
        ...
        if (fork_node) {
            ...
            if (!fork_to_as_of.empty()) fork_to_as_of.pop_front();
            for (auto it = fork_to_as_of.rbegin(); it != fork_to_as_of.rend(); it++)
                HASH_FUNCTION(it->hash, as_of_hash, as_of_hash);

            MERKLECPP_TRACE(
                { MERKLECPP_TOUT << " - as_of hash: " << as_of_hash.to_string(TRACE_HASH_SIZE) << std::endl; });
        ...
        for (auto it = root_to_fork.rbegin(); it != root_to_fork.rend(); it++) path.push_back(std::move(*it));

        return std::make_shared<Path>(leaf_node(index)->hash, index, std::move(path), as_of);
    }
    ...
    void serialise(std::vector<uint8_t>& bytes) {
        ...
        for (auto& n : leaf_nodes) n->hash.serialise(bytes);
        for (auto& n : uninserted_leaf_nodes) n->hash.serialise(bytes);
        ...
        if (!empty()) {
            ...
            MERKLECPP_TRACE(MERKLECPP_TOUT << to_string(TRACE_HASH_SIZE) << std::endl;);

            std::vector<Node*> extras;
            walk_to(min_index(), false, [&extras](Node*& n, bool go_right) {
                if (go_right) extras.push_back(n->left);
                return true;
            });

            for (size_t i = extras.size() - 1; i != SIZE_MAX; i--) extras[i]->hash.serialise(bytes);
        }
    }
    ...
    void serialise(size_t from, size_t to, std::vector<uint8_t>& bytes) {
        MERKLECPP_TRACE(MERKLECPP_TOUT << "> serialise from " << from << " to " << to << std::endl;);

        if ((from < min_index() || max_index() < from) || (to < min_index() || max_index() < to) || from > to)
            throw std::runtime_error("invalid leaf indices");

        serialise_uint64_t(to - from + 1, bytes);
        serialise_uint64_t(from, bytes);
        for (size_t i = from; i <= to; i++) leaf(i).serialise(bytes);
    ...
    const Hash& leaf(size_t index) const {
        ...
        if (index >= num_leaves()) throw std::runtime_error("leaf index out of bounds");
        if (index - num_flushed >= leaf_nodes.size())
    ...
    size_t num_leaves() const { return num_flushed + leaf_nodes.size() + uninserted_leaf_nodes.size(); }
    ...
    size_t size() {
        if (!uninserted_leaf_nodes.empty()) insert_leaves();
    ...
    size_t serialised_size() {
        ...
        if (!empty()) {
            walk_to(min_index(), false, [&num_extras](Node*&, bool go_right) {
                if (go_right) num_extras++;
                return true;
            });
        }

        return sizeof(leaf_nodes.size()) + sizeof(num_flushed) + leaf_nodes.size() * sizeof(Hash) +
               num_extras * sizeof(Hash);
    }
    ...
    size_t serialised_size(size_t from, size_t to) {
        ...
        return sizeof(leaf_nodes.size()) + sizeof(num_flushed) + (to - from + 1) * sizeof(Hash) +
               num_extras * sizeof(Hash);
    }
    ...
    mutable struct Statistics {
        ...
        std::string to_string() const {
            std::stringstream stream;
            stream << "num_insert=" << num_insert << " num_hash=" << num_hash << " num_root=" << num_root
                   << " num_retract=" << num_retract << " num_flush=" << num_flush << " num_paths=" << num_paths
   ...
   protected:
    ...
    void hash(Node* n, size_t indent = 2) const {
        ...
        while (!hashing_stack.empty()) {
            ...
            if (n->left && n->left->dirty)
                ...
                statistics.num_hash++;
                MERKLECPP_TRACE(MERKLECPP_TOUT
                                << std::string(indent, ' ') << "+ h(" << n->left->hash.to_string(TRACE_HASH_SIZE)
    ...
    void compute_root() {
        insert_leaves(true);
        if (num_leaves() == 0) throw std::runtime_error("empty tree does not have a root");
        assert(_root);
        assert(_root->invariant());
        if (_root->dirty) {
            hash(_root);
    ...
    void continue_insertion_stack(Node* n, Node* new_leaf) {
        while (true) {
            MERKLECPP_TRACE(MERKLECPP_TOUT << "  @ " << n->hash.to_string(TRACE_HASH_SIZE) << std::endl;);
            assert(n->invariant());
    ...
    Node* process_insertion_stack(bool complete = true) {
        MERKLECPP_TRACE({
            ...
            MERKLECPP_TOUT << "  X " << (complete ? "complete" : "continue") << ":" << nodes << std::endl;
        });
        ...
        while (!insertion_stack.empty()) {
            ...
            if (!complete && !result->is_full()) {
                MERKLECPP_TRACE(MERKLECPP_TOUT << "  X save " << result->hash.to_string(TRACE_HASH_SIZE) << std::endl;);
                return result;
            }
        }
    ...
    void insert_leaf(Node*& root, Node* n) {
        MERKLECPP_TRACE(MERKLECPP_TOUT << " - insert_leaf " << n->hash.to_string(TRACE_HASH_SIZE) << std::endl;);
        leaf_nodes.push_back(n);
    ...
    void insert_leaves(bool complete = false) {
        if (!uninserted_leaf_nodes.empty()) {
            ...
            // leaves to insert.
            for (auto& n : uninserted_leaf_nodes) insert_leaf(_root, n);
            uninserted_leaf_nodes.clear();
        }
        if (complete && !insertion_stack.empty()) _root = process_insertion_stack();
    }
};
...
static inline void sha256_compress_openssl(const HashT<32>& l, const HashT<32>& r, HashT<32>& out) {
    unsigned char block[32 * 2];
    memcpy(&block[0], l.bytes, 32);
    memcpy(&block[32], r.bytes, 32);

    SHA256_CTX ctx;
    if (SHA256_Init(&ctx) != 1) printf("SHA256_Init error");
    SHA256_Transform(&ctx, &block[0]);

    for (int i = 0; i < 8; i++) ((uint32_t*)out.bytes)[i] = convert_endianness(((uint32_t*)ctx.h)[i]);
}
...
/// @param out Output node hash
/// @note Some versions of OpenSSL may not provide SHA256_Transform.
static inline void sha256_openssl(const merkle::HashT<32>& l, const merkle::HashT<32>& r, merkle::HashT<32>& out) {
    uint8_t block[32 * 2];
    memcpy(&block[0], l.bytes, 32);
    memcpy(&block[32], r.bytes, 32);
...
/// use only.
static inline void sha256_compress_mbedtls(const HashT<32>& l, const HashT<32>& r, HashT<32>& out) {
    unsigned char block[32 * 2];
    memcpy(&block[0], l.bytes, 32);
    memcpy(&block[32], r.bytes, 32);
    ...
    for (int i = 0; i < 8; i++) ((uint32_t*)out.bytes)[i] = htobe32(ctx.state[i]);
}
...
/// @param r Right node hash
/// @param out Output node hash
static inline void sha256_mbedtls(const merkle::HashT<32>& l, const merkle::HashT<32>& r, merkle::HashT<32>& out) {
    uint8_t block[32 * 2];
    memcpy(&block[0], l.bytes, 32);
    memcpy(&block[32], r.bytes, 32);
